...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 31 17:22:27 2023
...
pet_images = [os.path.join(pet_folder, f) for f in os.listdir(pet_folder) if f.endswith('.nii') or f.endswith('.nii.gz')]
...
for dynamic_feature_name in ['fractional_occ_fpn_plus','fractional_occ_som_plus','fractional_occ_vis_minus','dwell_time_fpn_plus','dwell_time_vis_minus','ramdom']:
    ...
    # Load images
    pet_imgs = image.concat_imgs(pet_images)
    ...
    for i in range(pet_data_2d.shape[1]):
        correlation_coefficients[i], p_values[i] = pearsonr(pet_data_2d[:, i], dynamic_features)
    ...
    correlation_file_name = './results/example/nnc_correlation_'+ dynamic_feature_name +'_stat_map.nii.gz'
    correlation_img.to_filename(correlation_file_name)
    
    # Plotting the significant correlations with age
    # plotting.plot_stat_map(correlation_img,cmap='RdBu_r',
    #               threshold=0.1)  # Set threshold to 0 or a very low value to show smaller activations/deactivations
    
    
    
    #plotting.plot_glass_brain(correlation_img, colorbar=True, cmap='RdBu_r', vmax=0.5, vmin=-0.5,threshold=0)
    ...
    correlation_file_name = './results/example/nnc_correlation_'+ dynamic_feature_name +'_stat_map_20voxel_cluster.nii.gz'
    correlation_img.to_filename(correlation_file_name)
...
# Create binary masks for the specified regions
def create_binary_mask(atlas_data, labels):
    return np.isin(atlas_data, labels).astype(int)

# Load the atlas data
atlas_data = atlas_img.get_fdata()

# Generate binary masks for each color group
red_mask_data = create_binary_mask(atlas_data, [9, 10, 25, 26])
green_mask_data = create_binary_mask(atlas_data, [13, 14, 29, 30])
blue_mask_data = create_binary_mask(atlas_data, [15, 16, 31, 32])

# Create Nifti images for each mask
red_mask_img = image.new_img_like(atlas_img, red_mask_data)
green_mask_img = image.new_img_like(atlas_img, green_mask_data)
blue_mask_img = image.new_img_like(atlas_img, blue_mask_data)


# Threshold coefficients by p-value
significant_correlation = np.where(p_values < 0, correlation_coefficients, 0) # for the null template
# Convert the significant correlation values back into a Nifti image
correlation_img = masker.inverse_transform(significant_correlation)


# Plot the glass brain with overlays for each mask
display = plotting.plot_glass_brain(correlation_img, display_mode='lyrz', colorbar=False)
# Overlay the masks as contours
display.add_contours(red_mask_img, levels=[0.5], colors=['r'])
display.add_contours(green_mask_img, levels=[0.5], colors=['g'])
display.add_contours(blue_mask_img, levels=[0.5], colors=['b'])
